...
ggplot(flowercount.summary, aes(y=flower_number, x= paste(week_num, treatment), fill=plant)) +geom_col() + scale_x_discrete(guide = guide_axis(angle=90)) + scale_fill_manual(values = species_colors)
...
ggplot(flowercounts %>% left_join(rainout.treatments), aes(x=week_num, y=flower_number, color=treatment)) + facet_wrap(vars(plant), scales="free_y") + geom_boxplot() + scale_color_manual(values=dnet.color.palette)
```
...
stats.downsample <- tibble(permutation=rep(1:99, times=2), treatment=rep(c("control", "rainout"), each=99)) %>% 
