...
flowercounts<- read_sheet("1YR7lBNf5s9Drw9Ix61J8wUtKQO-XuZmXrRMhgktsc5k", sheet="2024") %>% filter(plant!="na") %>%  #2024-07-02, 4f - no flowers open wrote na
  ...
  group_by(date, week_num, transect, plot,transectplot, plant) %>% 
...
vrate.flowercount<- vrate.complete %>% 
                    group_by(date, week_num, start, end, transect, plot, transectplot, plant, insect_class) %>% #selects columns to keep in dataframe
...
#the zeros were placeholders, which meant that we did not observe an interaction. We want a dataframe with only observed interactions so that we can plot it and not have a ridiculous number of facets (only include facets with non-zero data)
