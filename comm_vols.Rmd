...
chemtable<- chemtable %>% rename(filtercount.floral="filter_count.floral") %>% rowwise() %>% mutate(filtercount.anyspecies="OK"%in% c_across(starts_with("filter_count")))

attr(chemtable, "min_ratio") <- 3

#TODO: filter ambient ratio needs to change if you want to get rare VOCs 
#add function that allows you to group ambient ratio by species so that VOCs are diluted
...
vol <- vol / as.numeric(meta$equil + meta$pumping, units="hours")# / meta$amount
```
